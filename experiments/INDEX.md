...
| [exp_05](exp_05_gtsrb_kdm_npc/DESIGN.md) | KDM vs. NPC en GTSRB (imágenes reales, 43 clases, sensibilidad de `sigma_mult`/escalamiento de `n_comp_final`) | 🔄 Fase A + A2 completas, Fase B pendiente de aprobación | KDM Cartesian sostiene su ventaja bajo ruido real de imagen y un espacio de salida ~2.3× mayor | Fase A: `lr_kdm=3e-3` (heredado de MNIST) **diverge catastróficamente** en GTSRB (dim_x 31× mayor); `lr_kdm=3e-4` converge perfectamente. Fase A2 (`lr_kdm=3e-4` fijo): `sigma_mult=0.5` da la misma accuracy (99.95%) con TV **19.4× mejor** (0.0025 vs. 0.0488) — nuevo ganador `search-lr3e4-sig05` | 2026-07-16 |
