...
TODOs a llenar una vez empaquetado y subido el dataset de Kaggle
(IMPLEMENTATION.md 1.2, bloqueado por internet al escribir esta plantilla):
  - EXPECTED_GLOBAL_SHA256: sha256 del MANIFEST.json del dataset subido.
  - Los conteos esperados de train/validate/test (linea ~123) -- GTSRB
    completo tiene ~39,209 imagenes de entrenamiento oficiales + 12,630 de
    test oficiales, pero el split real es el de gtsrb_split.json.gz
    (proporcion NPC estandar), hay que leerlo una vez materializado.
"""
...
EXPECTED_GLOBAL_SHA256 = "__TODO_FILL_AFTER_KAGGLE_UPLOAD__"
...
# TODO: reemplazar por los conteos reales una vez materializado el split
# (ver docstring del modulo).
for split_name in ("train", "validate", "test"):
    n = sum(len(fs) for _, _, fs in os.walk(f"{NPC_ROOT}/datasets/gtsrb/splits/instances/{split_name}"))
    print(f"[INFO] split {split_name}: {n} imagenes", flush=True)
    assert n > 0, f"Split {split_name} vacio"
